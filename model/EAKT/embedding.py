...
class Embedding(nn.Module):
    def __init__(self, n_questions, length, embedding_dim):
        ...
        self.length = length
    def forward(self, y ,device):  # shape of input: [batch_size, length, questions * 2]
        n_batch = y.shape[0]
        x = y[:, :, 0:self.n_questions] + y[:, :, self.n_questions:]
        p = torch.LongTensor([[i for i in range(self.length)] for j in range(n_batch)]).to(device)
        pos = self.pos_emb(p).to(device)
